...
static inline int should_filter_path(const char *path) {
   if (path[0] == '/' &&
       path[1] == 'p' &&
       path[2] == 'r' &&
       path[3] == 'o' &&
...
TRACEPOINT_PROBE(syscalls, sys_enter_openat) {
   ...
   if (should_filter_path(filename)) {
       return 0;
   }
...
TRACEPOINT_PROBE(syscalls, sys_enter_unlink) {
   ...
   bpf_probe_read_user_str(&pathname, sizeof(pathname), (void*)args->pathname);
...
TRACEPOINT_PROBE(syscalls, sys_enter_rename) {
   ...
   bpf_probe_read_user_str(&pathname, sizeof(pathname), (void*)args->oldname);
   if (should_filter_path(pathname)) {
       return 0;
   }


   bpf_probe_read_user_str(&pathname, sizeof(pathname), (void*)args->newname);
   if (should_filter_path(pathname)) {
       return 0;
   }
